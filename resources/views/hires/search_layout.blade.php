    ...
    <div class="row">
        <aside class="col-lg-3 col-md-3">
            ...
            <div class="box_style_cat">
                <ul id="cat_nav">
                    @foreach (config('tours.hires_types') as $type => $name)
                        <li>
                            <a href="{{ route('hires.index', array_merge(Request::all(), ['type' => $type])) }}" id="{{ Request::input('type') == $type || ($type == '' && !Request::input('type')) ? 'active' : '' }}">
                                {{ $name }} ({{ $type == '' || $hires['numberOfResults'] == 0 ? $total['numberOfResults'] : $hires['facetGroups'][0]['facets'][array_search($type, array_column($hires['facetGroups'][0]['facets'], 'name'))]['count'] }})
                            </a>
                        </li>
                    @endforeach
                </ul>
